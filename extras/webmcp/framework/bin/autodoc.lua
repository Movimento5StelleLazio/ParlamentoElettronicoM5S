    ...
    for filename in find_proc:lines() do
        ...
        local function entry_done()
            if #synopsis > 0 then
                ...
                local concatted_synopsis = string.gsub(table.concat(stripped_synopsis, " "), "[%s]+", " ")
                local func_call = string.match(concatted_synopsis, "^[A-Za-z0-9_, ]+= ?(.-) ?$")
                ...
                func_call = string.gsub(func_call,
                    "^([^({]*)[({].*[,;].*[,;].*[,;].*[)}]$",
                    function(base)
                        return base .. "{ ... }"
                    end)
...
for idx, key in ipairs(keys) do
    ...
    output('  <div class="short_synopsis"',
    ...
    output('  </div>\n')
    output('  <div id="autodoc_details_',
        idx,
