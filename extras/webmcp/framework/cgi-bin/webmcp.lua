...
do
    ...
    if not app_base then
        error("Failed to initialize autoloader " ..
    ...
    local function compose_path_string(base, path, key)
        return string.gsub(base .. table.concat(path, "/") .. "/" .. key, "/+", "/")
    end
    ...
    function autoloader_mt.__index(self, key)
        ...
        if merge then
            merge_path_string = compose_path_string(merge_base_path, path, file_key)
        end
...
-- execute configuration file
    ...
    elseif request.get_action() then
        ...
        request.get_404_route() and
                not file_exists(encode.action_file_path {
                    module = request.get_module(),
                    action = request.get_action()
                })
        then
            ...
            if not request.is_rerouted() then
                ...
                if routing_mode == "redirect" then
                    ...
                    for key, value in pairs(cgi.params) do
                        local status, stripped_key = string.match(key, "^_webmcp_routing%.([^%.]*)%.params%.(.*)$")
        ...
        if
        request.get_404_route() and
                not file_exists(encode.view_file_path {
                    module = request.get_module(),
                    view = request.get_view()
                })
        then
...
--end
    ...
    function(errobj)
        return {
            errobj = errobj,
            stacktrace = string.gsub(debug.traceback('', 2),
                "^\r?\n?stack traceback:\r?\n?", "")
        }
    end)
...
if not success then
    ...
    slot.select('system_error', function()
        if getmetatable(errobj) == mondelefant.errorobject_metatable then
            slot.put("<p>Database error of class <b>",
