...
-- helper function
local function get_tag_value(str, -- HTML document or document snippet
match_tag, -- tag name
...
local function tag_contents(str, match_tag)
    ...
    local function next_tag()
        local prefix
        tagpos, prefix, tag, pos = string.match(str,
            "()<(/?)([0-9A-Za-z:_-]+)[^>]*>()",
            pos)
        closing = (prefix == "/")
    end
...
function auth.openid.discover(args)
    ...
    while true do
        ...
        local location = string.match(headers,
        ...
        if location then
            ...
            local xrds_location = string.match(headers,
    ...
    elseif html_document then
        ...
        claimed_identifier = url
        op_endpoint = get_tag_value(html_document,
            "link", "rel", "openid2.provider", "href")
        op_local_identifier = get_tag_value(html_document,
