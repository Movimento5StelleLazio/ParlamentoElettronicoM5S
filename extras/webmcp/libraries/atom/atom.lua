...
do
    ...
    function create_new_type(name)
        local t = setmetatable({ methods = {}, getters = {}, name = name },
            type_mt)
    ...
    function has_type(value, t)
        ...
        lua_type == t or
                ...
                (lua_type == "string" and t == _M.string) or (lua_type == "number" and
                (t == _M.number or (t == _M.integer and (not (value <= 0 or value >= 0) or (value % 1 == 0 and
                        (value + 1) - value == 1 and
                        value - (value - 1) == 1)))))
    end
...
function fraction:new(numerator, denominator)
    if not ((numerator == nil or type(numerator) == "number") and
            (denominator == nil or type(denominator) == "number")) then
...
do
    ...
    function date.ymd_to_jd(year, month, day)
        ...
        local n1 = r100 % 4
        local jd = (c400 * n400 + c100 * n100 + c4 * n4 + c1 * n1 +
...
function date:load(str)
    ...
    elseif type(str) ~= "string" then
        ...
        local year, month, day = string.match(str, "^([0-9][0-9][0-9][0-9])-([0-9][0-9])-([0-9][0-9])$")
        if year then
            return date {
                year = tonumber(year),
...
function date:__tostring()
    if self.invalid then
        ...
        return string.format("%04i-%02i-%02i", self.year, self.month, self.day)
    end
end
        ...
        then
            ...
            then
                return timestamp:_create {
                    tsec = timestamp.ymdhms_to_tsec(args.year, args.month, args.day,
...
function timestamp:load(str)
    ...
    elseif type(str) ~= "string" then
        ...
        local year, month, day, hour, minute, second = string.match(str,
...
function timestamp:__tostring()
    if self.invalid then
        ...
        return string.format("%04i-%02i-%02i %02i:%02i:%02i",
...
function time:load(str)
    ...
    elseif type(str) ~= "string" then
        ...
        local hour, minute, second = string.match(str,
