...
function set_cookie(args)
    ...
    if args.domain then
        assert(string.find(args.path, "^[0-9A-Za-z%%/._~-]+$"),
...
if post_contenttype and (post_contenttype == "application/x-www-form-urlencoded" or
        string.match(post_contenttype, "^application/x%-www%-form%-urlencoded[ ;]")) then
    read_urlencoded_form(post_params, post_data)
elseif post_contenttype then
    local boundary = string.match(post_contenttype,
        '^multipart/form%-data[ \t]*;[ \t]*boundary="([^"]+)"') or string.match(post_contenttype,
    ...
    if boundary then
        ...
        for i, part in ipairs(parts) do
            ...
            while true do
                ...
                if string.find(string.lower(header),
                    "^content%-disposition:[ \t]*form%-data[ \t]*;") then
                    ...
                    local dummy, subpos = string.find(string.lower(header),
                        "^content%-type:[ \t]*")
                    if subpos then
