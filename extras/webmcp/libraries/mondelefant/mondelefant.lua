...
function class_prototype:get_db_conn()
    error("Method mondelefant class(_prototype):get_db_conn() " ..
...
--]] --
function class_prototype.list:get_reference_selector(ref_name, options, ref_alias, back_ref_alias)
    ...
    self.references[ref] = {
        ...
        -- TODO: support tuple keys
            ...
            if connected_by_table then
                selector:join(connected_by_table,
                    nil,
                    {
                        '$."$" = $."$"',
                        { connected_by_table },
