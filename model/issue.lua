...
Issue:add_reference {
    ...
    connected_by_this_key = 'issue_id',
    connected_by_that_key = 'member_id',
    ref = 'population_members_snapshot'
}
...
function Issue:get_search_selector(search_string)
    return self:new_selector():join('"initiative"', nil, '"initiative"."issue_id" = "issue"."id"'):join('"draft"', nil, '"draft"."initiative_id" = "initiative"."id"'):add_where { '"initiative"."text_search_data" @@ "text_search_query"(?) OR "draft"."text_search_data" @@ "text_search_query"(?)', search_string, search_string }:add_group_by('"issue"."id"'):add_group_by('"issue"."area_id"'):add_group_by('"issue"."policy_id"'):add_group_by('"issue"."state"'):add_group_by('"issue"."phase_finished"'):add_group_by('"issue"."created"'):add_group_by('"issue"."accepted"'):add_group_by('"issue"."half_frozen"'):add_group_by('"issue"."fully_frozen"'):add_group_by('"issue"."closed"'):add_group_by('"issue"."status_quo_schulze_rank"'):add_group_by('"issue"."cleaned"'):add_group_by('"issue"."snapshot"'):add_group_by('"issue"."latest_snapshot_event"'):add_group_by('"issue"."population"'):add_group_by('"issue"."voter_count"'):add_group_by('"issue"."admission_time"'):add_group_by('"issue"."discussion_time"'):add_group_by('"issue"."verification_time"'):add_group_by('"issue"."voting_time"')
    --:set_distinct()
end
