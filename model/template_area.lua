...
function TemplateArea.object_get:default_policy()
  return Policy:new_selector()
    :join("template_area_allowed_policy", nil, "template_area_allowed_policy.policy_id = policy.id")
    :add_where{ "template_area_allowed_policy.area_id = ? AND template_area_allowed_policy.default_policy", self.id }
