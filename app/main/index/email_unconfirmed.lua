if app.session.member.notify_email_unconfirmed then
    ...
    local current = Member:new_selector():add_where { "id = ?", app.session.member_id }:add_where("notify_email_unconfirmed NOTNULL"):add_where("notify_email_secret_expiry > now()"):optional_object_mode():exec()
