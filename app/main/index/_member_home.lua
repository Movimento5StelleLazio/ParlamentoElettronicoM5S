...
for i, unit in ipairs(units) do
    if member:has_voting_right_for_unit_id(unit.id) then
        ...
        local max_area_count = Area:new_selector():add_where { "area.unit_id = ?", unit.id }:add_where { "area.active" }:count()
        local more_area_count = max_area_count - area_count
        local delegated_count = Area:new_selector():add_where { "area.unit_id = ?", unit.id }:add_where { "area.active" }:left_join("membership", nil, { "membership.area_id = area.id AND membership.member_id = ?", member.id }):add_where { "membership.member_id ISNULL" }:join("delegation", nil, { "delegation.area_id = area.id AND delegation.truster_id = ?", member.id }):add_where { "delegation.trustee_id NOTNULL" }:count()
        ...
        ui.container {
            attr = { class = "area", style = "margin-top: 1ex; margin-left: 10px;" },
            content = function()
                ui.container {
                    attr = { class = "title" },
                    content = function()
