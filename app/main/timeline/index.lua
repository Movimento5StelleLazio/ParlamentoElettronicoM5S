...
for event, event_name in pairs(event_names) do
    ...
    if param.get("option_" .. event, atom.boolean) then
        ...
        if event == "draft_created" then
            tmp:reset_fields():add_field("max(timeline.occurrence)", "occurrence"):add_field("timeline.event", nil, { "grouped" }):add_field("timeline.issue_id", nil, { "grouped" }):add_field("draft.initiative_id", nil, { "grouped" }):add_field("max(timeline.draft_id)", "draft_id"):add_field("timeline.suggestion_id", nil, { "grouped" }):add_field("COUNT(*)", "count")
        else
            tmp:add_field("1", "count")
        end
...
if timeline_selector then
    ...
    outer_timeline_selector:add_field { "timeline.*" }:from({ "($)", { timeline_selector } }, "timeline"):add_order_by("occurrence DESC")
