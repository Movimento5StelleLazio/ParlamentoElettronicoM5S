  ...
  ui.container{ attr = { class = "span12 well-inside"}, content = function()
    ui.container{ attr = { class = "row-fluid" }, content = function()
      ui.container{ attr = { class = "span2 text-center"}, content = function()
      	if create then
		      if app.session.member.elected then
		        ui.link{  
		          module = "wizard_private", view = "shortcut",
      ...
      ui.container{ attr = { class = "span10" }, content = function()
        ...
        slot.put(" ")
        ui.link{ 
          module = "area_private", view = "show_ext_bs", id = area.id, params = { state = "admission" },
          text = _("#{count} new", { count = area.issues_new_count }) 
        }
        slot.put(" &middot; ")
        ui.link{ 
          module = "area_private", view = "show_ext_bs", id = area.id, params = { state = "discussion" },
          text = _("#{count} in discussion", { count = area.issues_discussion_count }) 
        }
        slot.put(" &middot; ")
        ui.link{ 
          module = "area_private", view = "show_ext_bs", id = area.id, params = { state = "verification" },
        ...
        slot.put(" &middot; ")
        ui.link{ 
          module = "area_private", view = "show_ext_bs", id = area.id, params = { state = "finished" },
          text = _("#{count} finished", { count = area.issues_finished_count }) 
        }
        slot.put(" &middot; ")
        ui.link{ 
          module = "area_private", view = "show_ext_bs", id = area.id, params = { state = "canceled" },
