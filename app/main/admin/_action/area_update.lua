...
old_default = AllowedPolicy:new_selector():add_where { "allowed_policy.area_id = ? AND allowed_policy.default_policy = 't'", area.id }:optional_object_mode():exec()
...
if default_policy_id and default_policy_id ~= -1 then
    pol = AllowedPolicy:new_selector():add_where { "allowed_policy.area_id = ? AND allowed_policy.policy_id = ?", area.id, default_policy_id }:optional_object_mode():exec()
    if pol then
